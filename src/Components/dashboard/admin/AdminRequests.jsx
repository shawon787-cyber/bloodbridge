...
export default function AdminRequests() {
  ...
  const statuses = ["Pending", "Approved", "Fulfilled", "Cancelled", "Rejected"];
  const urgencies = ["Urgent", "High", "Medium", "Low"];

  const totalRequests = mockBloodRequests.length;
  const pending = mockBloodRequests.filter((r) => r.status === "Pending").length;
  const urgent = mockBloodRequests.filter((r) => r.urgency === "Urgent").length;
  const fulfilled = mockBloodRequests.filter((r) => r.status === "Fulfilled").length;
  const cancelled = mockBloodRequests.filter((r) => r.status === "Cancelled").length;
  ...
  return (
    <div className="min-h-screen space-y-7">
      ...
      <section className="grid grid-cols-1 gap-5 md:grid-cols-3 xl:grid-cols-5">
        <StatCard title="Total Requests" value={totalRequests} icon={Droplets} color="#D62839" />
        <StatCard title="Pending" value={pending} icon={Clock3} color="#F59E0B" />
        <StatCard title="Urgent" value={urgent} icon={AlertTriangle} color="#EF4444" />
        <StatCard title="Fulfilled" value={fulfilled} icon={FulfilledIcon} color="#16A34A" />
        <StatCard title="Cancelled" value={cancelled} icon={CancelledIcon} color="#64748B" />
      </section>
      ...
      <SearchFilter
        ...
        filters={
          bloodGroupFilter || statusFilter || urgencyFilter || locationFilter
        }
        onFilterChange={() => {}}
        onClear={handleClear}
      >
